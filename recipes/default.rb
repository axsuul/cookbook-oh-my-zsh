...
node[:ohmyzsh][:users].each do |user|
  ...
  template_variables = { :theme => (node[:ohmyzsh][:theme]) }
  template_variables[:commands] = node[:ohmyzsh][user.to_sym][:commands] if node[:ohmyzsh][user.to_sym] and node[:ohmyzsh][user.to_sym][:commands]
